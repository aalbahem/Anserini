...
public class LogLogisticRelevanceModelRerankerTest {
  ...
  public void rerank() throws Exception {
    ...
    LogLogisticRelevanceModelReranker reranker = new LogLogisticRelevanceModelReranker(analyzer, CONTENT_FIELD, 10, 10, 0.5f, true,false);

    reranker.setIndexSearcher(context.getIndexSearcher());
    FeatureVector llgmodel = reranker.estimateRelevanceModel(ScoredDocuments.fromTopDocs(topDocs, indexSearcher),indexReader,false);
    ...
    llgmodel = reranker.estimateRelevanceModel(ScoredDocuments.fromTopDocs(topDocs,indexSearcher).scores, (String[])Arrays.copyOfRange(docs,0,2),indexReader,false);
    assertEquals(bananaFW/2.0f,llgmodel.getFeatureWeight("banana"),0.0);
  }
}